export function streamChat({ message, sessionId, userId, history, onChunk, onPlan, onStage, onSearchDone, onDone, onError }) {
  ...
  (async () => {
    try {
      const res = await fetch('/api/chat/stream', {
        ...
        body: JSON.stringify({ message, sessionId, userId, conversationHistory: history }),
        signal: ctrl.signal,
      });
      if (!res.ok) {
